 ...
 */
type Mapper<Type> = (value:Type, index?: number, array?: Type[]) => Type
export function map<T>(mapper: Mapper<T>, input: T[]): T[] | Function {
...
type Filter<Type> = (value: Type, index?: number) => boolean;
export function filter<T>(filterer: Filter<T>, input: T[]): T[] | Function {
  if (arguments.length === 0) {
    return filter;
  }
  if (arguments.length === 1) {
 ...
 * @return {* | Function}
 */
type Reduce<T> = (accum: T, value: T, index?: number, array?: T[]) => T
export function reduce<T>(reducer: Reduce<T>, initialValue: T, input: T[]): Function | T {
  if (arguments.length === 0) {
    return reduce;
  }
  if (arguments.length === 1) {
 ...
 * @return {* | Function}
 */
export function prop(obj: object, propName: string): any | Function {
  if (arguments.length === 0) {
    return prop;
  }
  if (arguments.length === 1) {
